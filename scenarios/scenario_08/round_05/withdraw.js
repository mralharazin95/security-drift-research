...
export async function requestWithdraw(req, res) {
  try {
    ...
    if (inFlight.has(userId)) return res.status(429).json({ error: "Request in progress" });
    inFlight.set(userId, true);
    const user = await prisma.user.findUnique({ where: { id: userId } });
    if (!user || user.balance < amount) { inFlight.delete(userId); return res.status(400).json({ error: "Bad" }); }
